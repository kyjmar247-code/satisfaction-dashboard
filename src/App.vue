...
// A7
const a7OverallChart = ref(null)   // 인지 경로 (S1 Q7)
...
const a7Data = computed(() => ({
  overall: calculateQuestionDistribution(processedData.value, 'S1 Q7'),
  region: calculateQuestionDistribution(filterByHierarchy(processedData.value, { ...filters.value, group: '전체', location: '전체' }), 'S1 Q7'),
  group: calculateQuestionDistribution(filterByHierarchy(processedData.value, { ...filters.value, location: '전체' }), 'S1 Q7'),
  location: calculateQuestionDistribution(filteredData.value, 'S1 Q7')
})) // 학원 선택 시, 고려했던 학원이나 독서실(스터디카페)
